...
int CriarRelatorio(char *pasta, int *vetor, int tamanho, int numero_teste,
                   int target[], int *alvos_encontrados, clock_t inicio,
                   clock_t termino, double tempos[]) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    fprintf(relatorio, "Tempo de Inicio: %lf\n",
...
int ExecutarTestesPesquisa(int *vetor, int tamanho, int numero_teste) {
  ...
  for (int i = 0; i < 6; i++) {
    inicio = clock();
    alvos_encontrados[i] = PesquisaSequencial(vetor, tamanho, target[i]);
    termino = clock();

    tempos[i] = ((double)(termino - inicio)) / CLOCKS_PER_SEC;

    printf("==TERMINO DA PESQUISA (%s)== \n", pasta);
    if (alvos_encontrados[i] != -1)
